...
function _next_target(creep: Creep) {
    if (creep.memory.target_type === "spawn") creep.memory.target_type = "extensions_not_full";
    else creep.memory.target_type = "spawn";
    // else  {
...
function _work(creep: Creep): void {
    ...
    if (!_.isEmpty(target)) {
        if (creep.memory.target_type !== "spawn") target = target[0];
        r = Utils._C(creep.name, creep.transfer(target, RESOURCE_ENERGY));
        if (r === ERR_NOT_IN_RANGE) creep.moveTo(target);
        else if (r === ERR_FULL) {
            if (creep.memory.target_type === "spawn") _next_target(creep);
        }
...
export function run(creep: Creep, opts?: {} | undefined) {
    ...
    if (!creep.memory.target_type) creep.memory.target_type = "spawn"; //? Transfer to spawn in priority and if full transfer to extensions.
