...
export function run(creep: Creep) {
    if (UPDATE(Game.spawns[creep.memory.spawn_name], ["construction_sites"])) {
        let to_build = Memory["rooms"][creep.room.name].structures['construction_sites']
        ...
        if (to_build.length < Config.REPAIR_WHEN_CONSTRUCTION_SITE_UNDER) {
            if (UPDATE(Game.spawns[creep.memory.spawn_name], ["to_repair"])) {
                creep.memory.target_type = 'repair'
                to_build = Memory["rooms"][creep.room.name].structures['to_repair']
            }
        }
