...
function _FIND_CONSTRUCTION_SITES_IDS(room: Room): Id<ConstructionSite<BuildableStructureConstant>>[] {
    return _.map(Game.spawns["Spawn1"].room.find(FIND_MY_CONSTRUCTION_SITES), (struct) => {
        return struct.id;
    });
}
...
function _FIND_NOT_FULL_CONTAINERS(room: Room): AnyStructure[] {
    return room.find(FIND_MY_STRUCTURES, {
        filter: (i: StructureExtension) =>
            i.structureType !== "extension" && i.structureType === "container" && i.store.getFreeCapacity(RESOURCE_ENERGY) > 0,
    });
}
...
function _GET_LVL_OF_ROOM(spawn: StructureSpawn) {
    return 300 + _.size(Memory["rooms"][spawn.room.name].structures["extensions"]) * 50 < 650 ? 1 : 2;
}
...
function UPDATE_IDS(spawn: StructureSpawn, update_list: string[]): boolean {
    let r = true;
    if (update_list.length >= 1)
        _.each(update_list, (up) => {
            if (!Memory["rooms"][spawn.room.name].updater[up] || Memory["rooms"][spawn.room.name].updater[up] !== Game.time) {
                ...
                switch (up) {
                    case "lvl": {
                        Memory["rooms"][spawn.room.name].lvl = _GET_LVL_OF_ROOM(spawn);
                        break;
                    }
                    case "spawn": {
                        Memory["rooms"][spawn.room.name].structures["spawn"] = _FIND_SPAWN(spawn.name).id;
                        break;
                    }
                    case "controller": {
        ...
        _.each(update_list, (up) => {
            if (!Memory["rooms"][spawn.room.name].updater[up] || Memory["rooms"][spawn.room.name].updater[up] !== Game.time) {
                ...
                switch (up) {
                    ...
                    case "spawn": {
                        Memory["rooms"][spawn.room.name].structures["spawn"] = _FIND_SPAWN(spawn.name);
                        break;
                    }
                    case "controller": {
                        Memory["rooms"][spawn.room.name].structures["controller"] = _FIND_CONTROLLER(spawn.room);
