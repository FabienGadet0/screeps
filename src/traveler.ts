 ...
 */
 ...
 export class Traveler {
    ...
    public static travelTo(creep: Creep, destination: HasPos|RoomPosition, options: TravelToOptions = {}): number | CreepMoveReturnCode{
        ...
        if (!this.samePos(state.destination, destination)) {
            if (options.movingTarget && state.destination.isNearTo(destination) && travelData.path) {
    ...
    public static addStructuresToMatrix(room: Room, matrix: CostMatrix, roadCost: number): CostMatrix {
        ...
        for (let structure of room.find<AnyStructure>(FIND_STRUCTURES)) {
